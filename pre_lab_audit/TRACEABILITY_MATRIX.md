...
| R-02 | **Self-tests must pass before any crypto operation** | `crates/pqc_crypto_module/src/self_test.rs`, `src/state.rs` | `self_test.rs` (KAT tests), `fips_readiness.rs`, `state.rs` (FSM transition tests) | `MOCK_AUDIT_REPORT.md` F-002, `FIPS_140_3_IG_CHECKLIST.md` #7 | PARTIAL | Self-tests execute and gate FSM via `initialize_approved_mode()`. KAT vectors are internal, not ACVP-sourced (F-002). |
...
| R-05 | **ML-KEM-768 encapsulate and decapsulate** | `crates/pqc_crypto_module/src/mlkem.rs` | `self_test.rs` (encaps/decaps KAT), `api_boundary.rs` | `MOCK_AUDIT_REPORT.md` F-001, `ACVP_DRY_RUN_PLAN.md` Section 4 | FAIL | Structural placeholder implementation via `encapsulate()` and `decapsulate()`. Self-test exercises the path but no production consumer. F-001 must be resolved: either complete FIPS 203 implementation or remove from approved list. |
...
|---|---|---|
| PASS | 6 | R-03, R-04, R-06, R-09, R-11, R-12 |
| PARTIAL | 5 | R-01, R-02, R-07, R-08, R-10 |
| FAIL | 1 | R-05 |
...
|---|---|---|
| R-05 (ML-KEM) | F-001 | Yes -- advertised approved algorithm is non-functional |
| R-02 (Self-tests / ACVP) | F-002 | Yes -- algorithm validation requires ACVP vectors |
