...
export default function Setup() {
  ...
  async function saveOrg() {
    ...
    if (!president.trim()) { setErr('El presidente es obligatorio (firma de actas)'); return }
    if (!secretary.trim()) { setErr('El secretario es obligatorio (firma de actas)'); return }
  ...
  async function createElection() {
    ...
    try {
      await submitProposal({
        proposer: `did:cerulean:${orgName.trim().toLowerCase().replace(/\s+/g, '-')}`,
          ...
          {STEPS.map((s) => (
            <div key={s.n} className="flex items-center gap-1">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold ${
                ...
                step === s.n ? 'bg-main-500 text-white' :
              ...
              <span className={`text-xs mr-3 ${step === s.n ? 'text-neutral-700 font-medium' : 'text-neutral-400'}`}>
                {s.label}
              </span>
              {s.n < 4 && <div className="w-8 h-px bg-neutral-200 mr-1" />}
            </div>
          ))}
        </div>
          ...
          {step === 1 && (
            <div className="bg-white rounded-xl border border-neutral-200 p-6 space-y-4">
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-1">Estos datos aparecen en actas y documentos oficiales.</p>
              </div>
              <div>
              ...
              <p className="text-[10px] text-neutral-400 text-center">Se creara automaticamente un canal DLT aislado para tu organizacion.</p>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="bg-white rounded-xl border border-neutral-200 p-6 space-y-4">
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-1">Cada persona recibe una wallet Ed25519 para firmar votos.</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-neutral-600 mb-1">Nombre</label>
                  <input className="w-full rounded-lg border border-neutral-200 px-3 py-2 text-sm" value={pName} onChange={(e) => setPName(e.target.value)} placeholder="Juan Perez" />
                </div>
                <div>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep(1)} className="flex-1 bg-neutral-100 text-neutral-600 py-2.5 rounded-lg text-sm font-semibold hover:bg-neutral-200 transition-colors">
                  Atras
                </button>
                <button onClick={() => { if (participants.length === 0) { setErr('Registra al menos un participante'); return }; setStep(3) }}
          ...
          {step === 3 && (
            <div className="bg-white rounded-xl border border-neutral-200 p-6 space-y-4">
              <div>
                ...
                <p className="text-sm text-neutral-500 mt-1">Define las unidades de tu organizacion. Puedes agregar mas despues.</p>
              </div>
              ...
              <p className="text-[10px] text-neutral-400">Puedes saltar este paso y crear la estructura despues en "Estructura".</p>

              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep(2)} className="flex-1 bg-neutral-100 text-neutral-600 py-2.5 rounded-lg text-sm font-semibold hover:bg-neutral-200 transition-colors">
                  Atras
                </button>
                <button onClick={() => { if (scopes.length > 0) saveScopes(); else setStep(4) }}
                  ...
                  {scopes.length > 0 ? `Crear ${scopes.length} unidades y continuar` : 'Saltar y continuar'}
                </button>
              </div>
            </div>
          )}

          {/* ── Step 4: First election ── */}
          {step === 4 && (
            <div className="bg-white rounded-xl border border-neutral-200 p-6 space-y-4">
              <div>
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 mb-1">Titulo de la votacion *</label>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep(3)} className="flex-1 bg-neutral-100 text-neutral-600 py-2.5 rounded-lg text-sm font-semibold hover:bg-neutral-200 transition-colors">
                  Atras
                </button>
