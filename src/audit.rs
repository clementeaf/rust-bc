...
mod tests {
    ...
    fn query_combined_filters() {
        ...
        let result = store
            .query(None, None, Some("org1"), Some(&AuditAction::BlockMined), 100)
            .unwrap();
    ...
    fn emit_if_present_with_some_appends() {
        let store: Option<Arc<dyn AuditStore>> =
            Some(Arc::new(MemoryAuditStore::new()));
        emit_if_present(&store, AuditAction::DidRevoked, "org2", Some("did:x".to_string()));
        let entries = store
            .as_ref()
            .unwrap()
