...
pub async fn store_create_organization(
    ...
    match &state.org_registry {
        None => Err(ApiError::NotFound {
            resource: "org_registry".to_string(),
        }),
        Some(reg) => {
...
pub async fn store_list_organizations(state: web::Data<AppState>) -> ApiResult<HttpResponse> {
    let trace_id = uuid::Uuid::new_v4().to_string();
    match &state.org_registry {
        None => Err(ApiError::NotFound {
            resource: "org_registry".to_string(),
        ...
        Some(reg) => {
            let orgs = reg.list_orgs().map_err(|e| ApiError::StorageError {
                reason: e.to_string(),
            })?;
...
pub async fn store_set_policy(
    ...
    match &state.policy_store {
        None => Err(ApiError::NotFound {
            resource: "policy_store".to_string(),
        }),
        Some(ps) => {
...
pub async fn store_get_policy(
    ...
    match &state.policy_store {
        ...
        Some(ps) => match ps.get_policy(&resource_id) {
            ...
            Err(_) => Err(ApiError::NotFound {
                resource: format!("policy {resource_id}"),
            }),
        },
    }
}
