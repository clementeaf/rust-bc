...
async fn two_nodes_gossip_event() {
    ...
    let received = tokio::time::timeout(Duration::from_secs(2), state_a.event_rx.recv()).await;
...
async fn boundary_sync_between_nodes() {
    ...
    let received = tokio::time::timeout(Duration::from_secs(2), state_a.boundary_rx.recv()).await;
...
async fn ping_pong() {
    ...
    let response =
        tokio::time::timeout(Duration::from_secs(2), p2p::decode_message(&mut stream)).await;
