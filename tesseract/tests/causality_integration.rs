...
fn causal_field_limits_propagation_by_light_cone() {
    ...
    assert!(
        field.get(origin).probability > 0.0,
        "origin should have probability"
    );
    ...
    assert!(
        far_p < EPSILON,
        "far cell should be outside light cone, got p={far_p}"
    );
}
...
fn light_cone_expands_with_ticks() {
    ...
    let origin = coord(5, 5, 5, 5);
    let eid = field
        .attest_causal(origin, b"event1", vec![], Dimension::Temporal, "val_t")
        .unwrap();
    ...
    let _eid2 = field
        .attest_causal(origin, b"event2", vec![eid], Dimension::Context, "val_c")
        .unwrap();

    let p_after = field.get(near).probability;
    assert!(
        p_after > p_before,
        "child event's cone should reach near cell, p_before={p_before}, p_after={p_after}"
    );
}
...
fn causal_violation_rejected() {
    ...
    assert!(
        result.is_none(),
        "event with unknown parent should be rejected"
    );
}
...
fn classical_field_unaffected() {
    ...
    let result = field.attest_causal(center, b"ev", vec![], Dimension::Temporal, "val_t");
