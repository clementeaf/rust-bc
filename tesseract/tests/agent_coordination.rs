...
fn step2_mutual_agreement_crystallizes() {
    ...
    assert!(record.contains("alice"), "Agreement records Alice: {}", record);
...
fn step3_conflicting_claim_rejected() {
    ...
    assert!(real_record.contains("alice"), "Real deal records Alice: {}", real_record);
    assert!(real_record.contains("bob"), "Real deal records Bob: {}", real_record);
    ...
    let charlie_primary = fake_cell.influences.iter()
    ...
    let real_primary = real_cell.influences.iter()
        .max_by(|a, b| a.weight.partial_cmp(&b.weight).unwrap())
...
fn step4_partition_independent_operation() {
    ...
    let mut alice_node = Node::new("alice-agent", size, Region {
        start: [0, 0, 0, 0],
        end: [size, size, size, size],
    });
    let mut bob_node = Node::new("bob-agent", size, Region {
        start: [0, 0, 0, 0],
        end: [size, size, size, size],
    });
    ...
    for _ in 0..50 { alice_node.evolve(); bob_node.evolve(); }

    assert!(alice_node.field.get(deal_coord).crystallized, "Deal crystallized on Alice");
    assert!(bob_node.field.get(deal_coord).crystallized, "Deal crystallized on Bob");
...
fn step5_reconnection_merges_state() {
    ...
    let region = Region { start: [0, 0, 0, 0], end: [size, size, size, size] };
    ...
    let alice_deal = Coord { t: 1, c: 1, o: 1, v: 1 };
    let bob_deal = Coord { t: 6, c: 6, o: 6, v: 6 };

    let ac = alice_node.field.get_mut(alice_deal);
    ac.probability = 1.0;
    ac.crystallized = true;
    ac.influences.push(Influence { event_id: "alice-solo-deal".into(), weight: 1.0 });

    let bc = bob_node.field.get_mut(bob_deal);
    bc.probability = 1.0;
    bc.crystallized = true;
    bc.influences.push(Influence { event_id: "bob-solo-deal".into(), weight: 1.0 });

    // Verify: each only knows their own deal
    assert!(alice_node.field.get(alice_deal).crystallized);
    assert!(!alice_node.field.get(bob_deal).crystallized, "Alice doesn't know Bob's deal");
    assert!(!bob_node.field.get(alice_deal).crystallized, "Bob doesn't know Alice's deal");
    ...
    assert!(alice_node.field.get(alice_deal).crystallized, "Alice keeps her deal");
    assert!(alice_node.field.get(bob_deal).crystallized, "Alice receives Bob's deal");
    assert!(bob_node.field.get(alice_deal).crystallized, "Bob receives Alice's deal");
    assert!(bob_node.field.get(bob_deal).crystallized, "Bob keeps his deal");

    // Influence provenance survives merge
    assert!(
        alice_node.field.get(bob_deal).influences.iter().any(|i| i.event_id.contains("bob")),
...
fn step6_destroyed_agreement_self_heals() {
    ...
    let owner_idx = if net.nodes[0].region.contains(coord) { 0 } else { 1 };
...
fn step7_audit_trail_via_influences() {
    ...
    net.distributed_seed(coord, "deal-001:buy:100", &[
        (0, "alice"),
        (0, "bob"),
        (1, "carol"),
    ]);
    ...
    let participants: Vec<&str> = cell.influences.iter()
        .map(|i| i.event_id.as_str())
        .collect();

    assert!(
        participants.iter().any(|p| p.contains("alice")),
        "Audit trail includes Alice: {:?}", participants
    );
    assert!(
        participants.iter().any(|p| p.contains("bob")),
        "Audit trail includes Bob: {:?}", participants
    );
    assert!(
        participants.iter().any(|p| p.contains("carol")),
        "Audit trail includes Carol: {:?}", participants
    );
...
fn step8_full_scenario_end_to_end() {
    ...
    assert!(net.get(deal_coord).crystallized, "Phase 1: Agreement crystallized");
    ...
    let real_record = real_cell.record();
    assert!(real_record.contains("alice"), "Phase 2: Real deal has Alice: {}", real_record);
    assert!(real_record.contains("bob"), "Phase 2: Real deal has Bob: {}", real_record);

    // Fraud's PRIMARY influence is mallory, not alice
    let fraud_primary = fraud_cell.influences.iter()
        .max_by(|a, b| a.weight.partial_cmp(&b.weight).unwrap())
    ...
    let owner = if net.nodes[0].region.contains(deal_coord) { 0 } else { 1 };
    ...
    for offset in 1..=3 {
        ...
        net.distributed_seed(nearby, &format!("context-{}", offset), &[(0, "alice"), (1, "bob")]);
    }
    net.run_to_equilibrium(10);

    net.nodes[owner].field.destroy(deal_coord);
    assert!(!net.nodes[owner].field.get(deal_coord).crystallized, "Phase 3: Destroyed");
    ...
    assert!(record.contains("alice") || record.contains("bob"),
        "Phase 4: Audit trail preserved after self-healing: {}", record);
}
