...
fn ten_nodes_clean() {
    ...
    assert!(
        r.convergence_ratio >= 0.9,
        "10 clean: {:.0}%",
        r.convergence_ratio * 100.0
    );
...
fn ten_nodes_adversarial() {
    let r = run_convergence_test(
        ...
        NetworkConfig::adversarial(),
        "adversarial",
        100,
    );
    assert!(
        r.convergence_ratio >= 0.5,
        "10 adversarial: {:.0}%",
        r.convergence_ratio * 100.0
    );
    assert_eq!(r.false_crystallization_rate, 0.0);
...
fn zero_false_crystallizations_all_profiles() {
    ...
    for (name, net) in [
        ...
        results.push(run_convergence_test(
            10,
            GossipConfig {
                fanout: 5,
                field_size: 6,
    ...
    // CSV
    let mut csv =
        String::from("nodes,profile,ticks,ratio,false_rate,msgs,msgs_per_node,dup,drop\n");
    for r in &results {
        csv.push_str(&format!(
            "{},{},{},{:.3},{:.4},{},{:.1},{:.3},{:.3}\n",
            r.num_nodes,
    ...
    for r in &results {
        assert_eq!(
            r.false_crystallization_rate, 0.0,
            "{} nodes / {}: false crystallization!",
            r.num_nodes, r.network_profile
        );
    }
}
