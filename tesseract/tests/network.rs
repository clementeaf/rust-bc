...
fn two_nodes_propagate_event() {
    ...
    let coord = Coord { t: 2, c: 3, o: 3, v: 3 };
    ...
    assert!(cell.record().contains("Alice"), "Record should contain event data");
}
...
fn event_crosses_node_boundary() {
    ...
    let near_boundary = Coord { t: 3, c: 3, o: 3, v: 3 };
    net.seed(near_boundary, "boundary-event");

    // Seed on node 1 (t=5), close to boundary
    let other_side = Coord { t: 5, c: 3, o: 3, v: 3 };
    net.seed(other_side, "other-side-event");

    net.run_to_equilibrium(10);

    // Both should crystallize
    assert!(net.get(near_boundary).crystallized, "Near-boundary event should crystallize");
    assert!(net.get(other_side).crystallized, "Other-side event should crystallize");
    ...
    assert!(mid_cell.probability > 0.0, "Midpoint should receive probability via boundary exchange");
}
...
fn four_nodes_independent_events() {
    ...
    let events = vec![
        (Coord { t: 1, c: 4, o: 4, v: 4 }, "ev-node0"),
        (Coord { t: 5, c: 4, o: 4, v: 4 }, "ev-node1"),
        (Coord { t: 9, c: 4, o: 4, v: 4 }, "ev-node2"),
        (Coord { t: 13, c: 4, o: 4, v: 4 }, "ev-node3"),
    ];
    ...
    for (coord, name) in &events {
        assert!(net.get(*coord).crystallized, "Event '{}' at {} should crystallize", name, coord);
    }
}
...
fn network_with_mapper() {
    ...
    let e1 = Event { id: "tx-001".into(), timestamp: 60, channel: "payments".into(), org: "alice".into(), data: "Alice→Bob:10".into() };
    let e2 = Event { id: "tx-002".into(), timestamp: 300, channel: "payments".into(), org: "bob".into(), data: "Bob→Carol:5".into() };
    ...
    assert!(net.get(c1).crystallized, "Event 1 should crystallize in network");
    assert!(net.get(c2).crystallized, "Event 2 should crystallize in network");
}
...
fn node_destruction_does_not_affect_other_nodes() {
    ...
    assert!(net.get(ev1).crystallized, "Event on other node should survive destruction");
}
...
fn distributed_seed_both_parties_see_event() {
    ...
    assert!(record.contains("alice") || record.contains("bob"),
        "Record should reference at least one party: {}", record);
}
...
fn distributed_seed_stronger_than_single() {
    ...
    assert!(dist_cell.crystallized, "Distributed seed should crystallize");
    ...
    assert!(
        ...
        single_cell.influences.len(), dist_cell.influences.len()
    );
}
...
fn distributed_seed_self_heals_from_both_sides() {
    ...
    net.distributed_seed(
        Coord { t: 3, c: 4, o: 3, v: 3 }, "related-1",
        &[(0, "alice"), (1, "bob")],
    );
    net.distributed_seed(
        Coord { t: 4, c: 3, o: 3, v: 3 }, "related-2",
        &[(0, "alice"), (1, "bob")],
    );
    net.run_to_equilibrium(10);
...
fn partition_reconciliation_preserves_both_sides() {
    ...
    let coord_a = Coord { t: 1, c: 1, o: 1, v: 1 };
    let coord_b = Coord { t: 6, c: 6, o: 6, v: 6 };
    ...
    cell_a.influences.push(Influence { event_id: "ev-A".into(), weight: 1.0 });

    let cell_b = field_b.get_mut(coord_b);
    cell_b.probability = 1.0;
    cell_b.crystallized = true;
    cell_b.influences.push(Influence { event_id: "ev-B".into(), weight: 1.0 });
    ...
    assert!(node_a.field.get(coord_a).crystallized, "A's crystal survives on A");
    assert!(node_a.field.get(coord_b).crystallized, "B's crystal arrives on A");
    assert!(node_b.field.get(coord_a).crystallized, "A's crystal arrives on B");
    assert!(node_b.field.get(coord_b).crystallized, "B's crystal survives on B");

    // Influences merged correctly
    assert!(node_a.field.get(coord_b).influences.iter().any(|i| i.event_id == "ev-B"));
    assert!(node_b.field.get(coord_a).influences.iter().any(|i| i.event_id == "ev-A"));
}
